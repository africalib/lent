(function () {
  const name = "goodsSharingListPage";
      ...
      resolve({
        ...
        methods: {
          writtenManager: function (mode, seq, title, content, regId) {
            ...
            let t = this;
            mainPage.headToPage("goods-sharing-page");
            mainPage.requestMode = regId == mainPage.userInfo.mbrId ? "edit" : mode;

            if (mode != "new") {
              findChildren();
              function findChildren() {
                let children = vue_findChildren(mainPage.$children, "goodsSharing", false);
                if (!children) {
                  setTimeout(function () {
                    findChildren();
                  }, 100);
